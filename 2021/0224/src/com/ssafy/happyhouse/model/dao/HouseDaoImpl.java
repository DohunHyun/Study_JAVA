...
import com.ssafy.happyhouse.view.SubwayView;
...
//		}
		...
		if(dong!=null) {	
			...
			for (HouseDeal deal : search) {
				if(deal.getAptName() == null) {} // 추가. 아파트 이름이 NULL 인 경우를 제외.
				else if(deal.getAptName().contains(aptName)) {
					finds.add(deal);
				}
			}
		} else {
	...
	public HouseDeal search(int no) {
		...
		for(int i=0; i<search.size(); i++) {
			if(search.get(i).getNo() == no) return search.get(i);
			SubwayView subwayView = new SubwayView(search.get(i).getDong()); // 지하철 정보 창 띄우기
		}
